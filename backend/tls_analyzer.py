...
#         def debug_print_scan_results(scanner_result):
#             """
#             打印 SSLyze 扫描结果的全部字段，包括各命令对象的状态、结果和错误信息。
#             """
#             logging.info("==== DEBUG: Full ScanResult Start ====")
#             attempts = getattr(scanner_result, "scan_result", None)
#             if not attempts:
#                 logging.info("No scan_result present.")
#                 return
...
#                 "heartbleed", "robot", "tls_compression",
#                 "session_renegotiation", "certificate_info"
#             ]:
#                 attempt = getattr(attempts, attr, None)
#                 if attempt is None:
#                     logging.info(f"{attr}: None")
#                     continue

#                 logging.info(f"--- {attr} ---")
#                 logging.info(f"Status: {attempt.status}")

#                 # 尝试打印 result 对象里的所有字段
...
#                 if error_reason:
#                     logging.info(f"Error reason: {error_reason}")

#                 error_trace = getattr(attempt, "error_trace", None)
#                 if error_trace:
#                     logging.info(f"Error trace: {str(error_trace)[:500]}")
#             logging.info("==== DEBUG: Full ScanResult End ====")

#         # for res in scanner.get_results():
#         #     logging.info(f"Scan status: {res.scan_status}")
#         #     debug_info.append(f"Scan status: {res.scan_status}")
#         #     logging.info(f"Connectivity: {res.connectivity_result}")
#         #     debug_info.append(f"Connectivity: {res.connectivity_result}")
#         for res in scanner.get_results():
#             logging.info(f"Scan status: {res.scan_status}")
#             logging.info(f"Connectivity: {res.connectivity_result}")

#             # 调试打印
#             debug_print_scan_results(res)
...
#                         findings.append({"protocol": label, "status": "COMPLETED", "accepted_cipher_suites": suites})
#                         debug_info.append(f"{label}: {len(suites)} cipher suites")
#                     # Heartbleed
#                     elif hasattr(attempt.result, "is_vulnerable_to_heartbleed"):
#                         findings.append({
#                             "protocol": label,
#                             "status": "COMPLETED",
#                             "vulnerable": attempt.result.is_vulnerable_to_heartbleed
#                         })
#                     # ROBOT
#                     elif hasattr(attempt.result, "result"):
#                         findings.append({
#                             "protocol": label,
#                             "status": "COMPLETED",
#                             "result": str(attempt.result.result)
#                         })
#                     # TLS Compression
#                     elif hasattr(attempt.result, "compression_supported"):
#                         findings.append({
#                             "protocol": label,
#                             "status": "COMPLETED",
#                             "compression_supported": attempt.result.compression_supported
#                         })
#                     # Session renegotiation
#                     elif hasattr(attempt.result, "is_vulnerable_to_client_initiated_renegotiation"):
#                         findings.append({
#                             "protocol": label,
#                             "status": "COMPLETED",
#                             "client_renegotiation_vulnerable": attempt.result.is_vulnerable_to_client_initiated_renegotiation,
#                             "secure_renegotiation_supported": getattr(attempt.result, "secure_renegotiation_supported", None)
#                         })
#                     # Certificate info
#                     elif hasattr(attempt.result, "certificate_chain"):
#                         chain_info = []
#                         for cert in attempt.result.certificate_chain:
#                             chain_info.append({
